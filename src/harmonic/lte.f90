...
subroutine calculate_speed_sound(structure,supercell, &
  ...
  do
    ...
    do i=1,max_samples
      ...
      dot_prod = abs(cmplx( &
         & mat(frequencies_polarisations%polarisation_vectors) &
         & * q_unit))
...
function evaluate_freqs_on_grid(supercell,force_constants) &
  ...
  do gvector=1,supercell%sc_size
    ...
    output%dynamical_matrices(:,:,gvector) = construct_dynamical_matrix( &
       & vec(dble(supercell%gvectors(gvector))),                         &
       & supercell, force_constants, delta_prim)
    ...
    do mode=1,supercell%no_modes_prim
      do atom=1,supercell%no_atoms
        ...
        if (gvector == gvector_p) then
          ...
          output%displacements(:,atom,mode,gvector) = &
             & sqrt(2.0)*real(cmplx(non_mr_pol_vec*exp_iqr(rvector)))
          output%k_displacements(:,atom,mode,gvector) = &
             & sqrt(2.0)*real(cmplx(kpol_vec*exp_iqr(rvector)))
          output%polarisation_vectors(:,atom,mode,gvector) = &
             & sqrt(2.0)*cmplx(non_mr_pol_vec)
          
          output%displacements(:,atom,mode,gvector_p) = &
             & sqrt(2.0)*aimag(cmplx(non_mr_pol_vec*exp_iqr(rvector)))
          output%k_displacements(:,atom,mode,gvector_p) = &
             & sqrt(2.0)*aimag(cmplx(kpol_vec*exp_iqr(rvector)))
...
subroutine fourier_interpolation(dyn_mats_ibz,structure,temperature, &
    ...
    do j=1,size(qpoints_ibz(i)%gvectors)
      ...
      do atom_1=1,structure%no_atoms
        ...
        do atom_2=1,structure%no_atoms
          ...
          dyn_mats_grid(mode_2p:mode_2p+2,mode_1p:mode_1p+2,k) = cmplx( &
             &   rotations_cart(l)                                      &
             & * mat(dyn_mats_ibz(mode_2:mode_2+2,mode_1:mode_1+2,i))   &
  ...
  do i_cell=1,structure_grid%sc_size
    do i_grid=1,structure_grid%sc_size
      k_dot_r = structure_grid%gvectors(i_grid) &
