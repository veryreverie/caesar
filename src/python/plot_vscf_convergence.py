...
def main():
  ...
  for x,y in zip(xs,ys):
    for j in range(len(x)):
      dxs[j].append(x[j]-xs[min_i][j])
      dys[j].append(y[j]-xs[min_i][j])
  
  handles = []
  labels = []
  line = mlines.Line2D([],[],marker='o',color='none',markerfacecolor=[0,0,0])
  handles.append(line)
  labels.append('Input Coefficients')
  line = mlines.Line2D([],[],marker='o',color='none',markerfacecolor='none',markeredgecolor=[0,0,0])
  handles.append(line)
