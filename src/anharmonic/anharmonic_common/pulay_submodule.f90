submodule (caesar_pulay_module) caesar_pulay_submodule
...
module procedure precondition_errors
...
module procedure regularised_invert
  type(SymmetricEigenstuff), allocatable :: estuff(:)
...
module procedure self_consistency_error
  output = maxval(abs(dble( this%fs_(this%i()) &
                        & - this%xs_(this%i()) )))
end procedure

module procedure coefficient_change
  integer :: i,j
...
module procedure free_energy_change
  integer :: i,j
...
module procedure example_function
...
module procedure example_free_energy
  output = sum(input)
end procedure

module procedure pulay_solver_example
